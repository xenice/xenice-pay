...
class Xunhupay
{
    ...
    public function execute($args)
    {
        ...
        $data=[
            ...
            /*
            'plugins'   => $args['user_id'],//必须的，根据自己需要自定义插件ID，唯一的，匹配[a-zA-Z\d\-_]+
            'trade_order_id'=> $args['trade_order_id'], //必须的，网站订单ID，唯一的，匹配[a-zA-Z\d\-_]+
            'payment'   => $args['payment'],//必须的，支付接口标识：wechat(微信接口)|alipay(支付宝接口)
            'total_fee' => $args['total_fee'],//人民币，单位精确到分(测试账户只支持0.1元内付款)
            'title'     => $args['title'], //必须的，订单标题，长度32或以内
            'notify_url'=>  admin_url('admin-ajax.php') . '?action=xunhupayNotify', //必须的，支付成功异步回调接口
            'return_url'=> $args['return_url'],//必须的，支付成功后的跳转地址
            'callback_url'=> $args['callback_url'],//必须的，支付发起地址（未支付或支付失败，系统会会跳到这个地址让用户修改支付信息）
            'nonce_str' => $args['nonce']//必须的，随机字符串，作用：1.避免服务器缓存，2.防止安全密钥被猜测出来*/
        ];
        
        $data = array_merge($data, $args);
        
        $data['hash']     = $this->generateHash($data, $this->appsecret);
        ...
        try {
            ...
            $hash         = $this->generateHash($result, $this->appsecret);
    ...
    public function verify()
    {
        ...
        //APP SECRET
        $hash = $this->generateHash($data, $this->appsecret);
        if($data['hash']!=$hash){
            return;
        }
        ...
        /*
        if(!take('not_log_buy')){
            if(empty($data['plugins']) || !is_numeric($data['plugins'])){
                echo 'failed';exit;
            }
        }*/
        
        $nonce = $data['nonce_str'];

        //商户订单ID

        if($data['status']=='OD'){
            $rt = [
                'order_sn'=>$data['trade_order_id'],
                'outer_order_sn'=>$data['transaction_id']
            ];
            return $rt;
            //Theme::use('order')->setPaid($trade_order_id, $data['transaction_id']);
            //Theme::use('auth')->addByOrder($trade_order_id);
    ...
    public function generateHash(array $datas,$hashkey)
    {
        ksort($datas);
        reset($datas);
